...
static std::string row(const std::string& lbl, const std::string& val) {
    int lbl_display = unicode_display_width(lbl);
    int padding = label_width - lbl_display;
    if (padding < 0) padding = 0;
    return std::string(padding, ' ') + lbl + " │ " + val + "\n";
}
...
static std::string pad_left(const std::string& s, int width) {
    int display_len = unicode_display_width(s);
    int padding = width - display_len;
    if (padding < 0) padding = 0;
